                ...
                <div class="col-md-12">
                    ...
                    @endif
                        ...
                        @if(count($errors) > 0)
                                ...
                                @endif
                        <hr>
                        <form enctype="multipart/form-data" action="{{ url('dashboard/course/view/'.$course->id.'/assignment/submit/'.$assignment->id.'/store') }}" method="POST">
                            @csrf
                            
                            <h4>Assignment Title: {{ $assignment->assignment_title }}</h4>
                            <hr>
                            <p>Total Marks: {{ $assignment->assignment_total_marks }}</p>
                            <p>Deadline: {{ $assignment->assignment_deadline }}</p>
                            
                            <div class="form-group">
                                <input type="file" name="assignment_file_student" class="form-control-file text-primary" id="assignment_file_student">
                                <hr>
                                <input type="submit" value="Submit Assignment" class="mt-1 form-control-file btn btn-sm btn-primary" id="assignment_file_student">
                            </div>
